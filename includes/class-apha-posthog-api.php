<?php
/**
 * AG Data for PostHog PostHog API Client.
 *
...
class APHA_PostHog_API {
	...
	private function post( $url, $payload, $blocking ) {
		...
		if ( is_wp_error( $response ) ) {
			wc_get_logger()->error( 'PostHog API error - ' . $response->get_error_message(), array( 'source' => 'ag-data-for-posthog' ) );
			return false;
		}
		...
		if ( $blocking ) {
			...
			if ( 200 !== $response_code ) {
				wc_get_logger()->error(
					...
					array( 'source' => 'ag-data-for-posthog' )
				);
				return false;
			}
